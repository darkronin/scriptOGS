...
for(var i = 0; i<f.length;i++){ if( f[i].title == '' ) continue; if(f[i])if(f[i].parentElement)if(f[i].parentElement.children[2]) if(awk.indexOf( f[i].parentElement.children[2].innerText ) == -1 ) continue; if(!f[i].parentElement.classList.contains('Player')) continue;  j = histoneartitle.indexOf( f[i].title ); if( j == -1 ){ histoneartitle.push( f[i].title); histonear.push(1); histoneari.push(i); } else { histonear[j]++;}}  console.log('Around your rank:',histonear, histoneartitle);
...
;for(var u = 0; u<histonear.length;u++){ /*console.log(u, histoneari[u], histoneartitle[u]);*/ hh3  = Object.assign(document.createElement('span')); hh3.style.width='30px';hh3.style.height='30px'; zz=f[histoneari[u]-0+histoflag.length].firstElementChild.cloneNode();  hh3.appendChild(zz); hh3.className = 'f16'; hh3.title = histoneartitle[u]; hh4 = Object.assign(document.createElement('span')); hh4.innerText = ' '+ histonear[u]; 
if(zz != undefined) { hh3.onclick=function(ev){var ij=histoneartitle.indexOf(this.title),ijc=histonear[ij],ijr=parseInt(Math.random() * ijc); var fc = -1;for(var i = 1; i<f.length;i++) { if(f[i].title==this.title){  if(fc == ijr){f[i].parentElement.children[1].scrollIntoView(); f[i].parentElement.style.border = 'dotted';} fc++; } }} }; 
yy.appendChild(hh3);yy.appendChild(hh4); yy.appendChild(document.createElement('br')); } 
